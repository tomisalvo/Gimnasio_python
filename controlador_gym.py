...
import sqlite3
from decoradores import Registro_log
import logging
from observador import  Observador_alta
...
class Ingreso:
    ...
    def ingresar(self):
        ...
        if not re.match(r"^\d{8}$", cliente):
            ...
            logging.error("DNI inválido: debe tener exactamente 8 dígitos")  # insertamos en el registro
            self.entry_dni.delete(0, END)
            return False  # corto la función si el DNI no es válido
        ...
        if resultado:
                ...
                logging.error(f"Acceso denegado para DNI {cliente}")  # insertamos en registro
                self.entry_dni.delete(0, END)
                return False
...
class Alta_alumno:
    ...
    def __init__(self, entry_dni, entry_nombre, entry_apellido, entry_direccion, entry_telefono):       
        ...
        self.crud = Crud()
        self.observador = Observador_alta()
    ...
    def guardar_alumno(self):
        ...
        try:
            ...
            # Notificar al observador SOLO si el alta fue exitosa
            cliente = {"nombre": nombre, "apellido": apellido, "dni": dni}
            self.observador.actualizar(cliente)
